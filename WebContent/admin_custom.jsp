	...
	<body>
						...
						<div class="col-lg-12 grid-margin stretch-card">
							<div class="card">
								<div class="card-body">
									...
									<ul class="list-star">
					                    <li>Add a new custom</li>
				                    </ul>
				                    <form action="/MyWebsite/admin/custom/add" method="post" id="addcustom" enctype="multipart/form-data">
					                    <input type="text" placeholder="Custom" class="myform bg-transparent border-0" name="customname" id="name">
										<input type="text" placeholder="Introduction" class="myform bg-transparent border-0" name="introduction" id="introduction">
									...
									<ul class="list-star">
					                    <li>Delete a custom</li>
				                    </ul>
				                    <form action="/MyWebsite/admin/custom/delete" method="post" id="delcustom">
					                    <input type="text" class="myform bg-transparent border-0" placeholder="Custom" name="customname" id="delname">
									...
									<ul class="list-star">
					                    <li>Alter a custom's introduction</li>
				                    </ul>
				                    <form action="/MyWebsite/admin/custom/alter" method="post" id="altcustom">
					                    <input type="text" class="myform bg-transparent border-0" placeholder="Custom" name="customname" id="altname">
					                    <input type="text" placeholder="New introduction" class="myform bg-transparent border-0" name="introduction" id="altintroduction">
									...
									<p class="card-description"> ht_custom <code>.table</code></p>
									<table class="mytable">
										<thead>
											<tr>
												<th>No</th>
										...
										%>
											<tr>
												<td><%out.print(i);%></td>
												<td><%=custom.getCustom_name() %></td>
												<td><%=custom.getCustom_introduction()%></td>
												<%String fullpath = custom.getCustom_pic_path();%>
		...
		<script type="text/javascript">
			...
			$("#addbtn").click(function() {
				...
				var addintroduction = $("#introduction").val();
				var addmsg = $("#addmsg");
				if (isEmpty(addname)) {
					addmsg.html("Please enter custom's name");
					return;
				}
				if (isEmpty(addintroduction)) {
			...
			$("#delbtn").click(function() {
				...
				var delmsg = $("#delmsg");
				if (isEmpty(delname)) {
					delmsg.html("Please enter a custom!");
					return;
				}
				if (!isExist(delname)) {
					delmsg.html("The custom does not exist!");
					return;
				}
				$("#delcustom").submit();
			});
			...
			$("#altbtn").click(function() {
				...
				var altintroduction = $("#altintroduction").val();
				var altmsg = $("#altmsg");
				if (isEmpty(altname)) {
					altmsg.html("Please enter custom's name");
					return;
				}
				if (isEmpty(altintroduction)) {
					altmsg.html("Please enter introduction!");
					return;
				}
				if (!isExist(altname)) {
					altmsg.html("The custom does not exist!");
					return;
				}
				$("#altcustom").submit();
			});
			...
			function isExist(x){
				...
				for(Custom custom:customs){
				%>
				var length = <%=customs.size()%>;
				var i = 0;
				for (; i < length; i++) {
					if (x == "<%=custom.getCustom_name()%>")
						flag = 1;
				};
				<%	
				}
